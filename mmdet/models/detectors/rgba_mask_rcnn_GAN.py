...
class FRGBARCNNGAN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 mask_gan=None):
        super(FRGBARCNNGAN, self).__init__(
            backbone=backbone,
            neck=neck,
    ...
    def forward_train_alpha(self,
        ...
        if self.with_mask:
            ...
            mask_pred = self.mask_head(mask_feats).tanh()

            gt_rgbs = []
            standard_imgs = (img * self.img_std + self.img_mean - 128)/ 128
            for standard_img in standard_imgs:
                gt_rgbs.append(standard_img.clamp_(-1, 1))
            ...
            loss_rgba = self.mask_head.loss(mask_pred, mask_targets, torch.ones_like(mask_pred))

            losses.update(loss_rgba)
            # visualize the loss
            for name, value in loss_rgba.items():
                self.visualize_value(value.data, 'Loss/train_' + str(name))
            ...
            # soft_mask = torch.rand_like(mask_targets, dtype=mask_targets.dtype, device=mask_targets.device) / 1000 + 0.999
            # mask_targets_label = mask_targets
            # mask_targets = mask_targets * soft_mask * 2 - 1
            if b > 0:
                self.visualize_img(mask_pred[:,0,:,:].view(b, 1, h, w), 'mask_pred', mean=[0.5], std=[0.5])
                self.visualize_img(mask_targets[:,0,:,:].view(b, 1, h, w), 'mask_targets', mean=[0.5], std=[0.5])
                self.visualize_img(mask_pred[:,1:,:,:].view(b, 3, h, w), 'rgb_pred', mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
                self.visualize_img(mask_targets[:,1:,:,:].view(b, 3, h, w), 'rgb_targets', mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
            # add mask gan loss for the network
            if self.with_mask_gan:
            #     # real = torch.zeros_like(mask_pred)
            #     # fake = torch.zeros_like(mask_pred)
                mask_label = torch.zeros((b, cls_score.size(1), h, w)).to(mask_pred.device)
                mask_label[inds, pos_labels, :, :] = mask_targets[:,0,:,:].view(b, h, w)
                real = torch.cat([mask_label, mask_targets], dim=1)
                fake = torch.cat([mask_label, mask_pred], dim=1)
                loss_mask_d = self.mask_gan.optimizer_d(real, fake)
                # visualize the loss
                for name, value in loss_mask_d.items():
                    self.visualize_value(value.data, 'Loss/train'+str(name))
                loss_mask_g = self.mask_gan.g_loss(real, fake)
                losses.update(loss_mask_g)
                # visualize the loss
                for name, value in loss_mask_g.items():
                    self.visualize_value(value.data, 'Loss/train_' + str(name))
