...
class FMaskRCNNGAN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 mask_gan=None):
        super(FMaskRCNNGAN, self).__init__(
            backbone=backbone,
            neck=neck,
    ...
    def forward_train_alpha(self,
        ...
        if self.with_mask:
            ...
            mask_pred_label = torch.nn.functional.tanh(mask_pred[inds, pos_labels].squeeze(1))
            ...
            if b > 0:
                self.visualize_img(mask_pred_label.view(b, 1, h, w), 'mask_pred', mean=[0.5], std=[0.5])
                self.visualize_img(mask_targets.view(b, 1, h, w), 'mask_targets', mean=[0.5], std=[0.5])
            # add mask gan loss for the network
            if self.with_mask_gan:
                # real = torch.zeros_like(mask_pred)
                # fake = torch.zeros_like(mask_pred)
                mask_label = torch.zeros_like(mask_pred)
                mask_label[inds, pos_labels, :, :] = mask_targets_label.view(b, h, w)
                real = torch.cat([mask_label, mask_targets.view(b, 1, h, w)], dim=1)
                fake = torch.cat([mask_label, mask_pred_label.view(b, 1, h, w)], dim=1)
                # fake[inds, pos_labels, :, :] = mask_pred_label.view(b, h, w)
