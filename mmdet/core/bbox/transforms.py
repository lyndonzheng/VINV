...
def occ2result(occs, labels, num_classes):
    """Convert binary occlusion results to a list of numpy arrays"""
    if occs.shape[0] == 0:
        return [np.zeros((0, 2), dtype=np.float32) for _ in range(num_classes - 1)]
    else:
        occs = occs.cpu().numpy()
        labels = labels.cpu().numpy()
        return [occs[labels == i, :] for i in range(num_classes - 1)]
