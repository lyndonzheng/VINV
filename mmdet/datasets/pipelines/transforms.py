...
class Resize(object):
    ...
    def _resize_f_rgb(self, results):
        if self.keep_ratio:
            f_rgbs = [
                mmcv.imrescale(f_rgb, results['scale_factor']) for f_rgb in results['f_rgbs']
            ]
        else:
            rgb_size = (results['img_shape'][1], results['img_shape'][0])
            f_rgbs = [
                mmcv.imrescale(f_rgb, rgb_size) for f_rgb in results['f_rgbs']
            ]
        results['f_rgbs'] = f_rgbs

    def _resize_depth(self, results):
        if self.keep_ratio:
            results['depth'] = mmcv.imrescale(results['depth'], results['scale_factor'], interpolation='bilinear')
        else:
            depth_size = (results['img_shape'][1], results['img_shape'][0])
            results['depth'] = mmcv.imrescale(results['depth'], depth_size, interpolation='bilinear')

    def _resize_f_depth(self, results):
        if self.keep_ratio:
            f_depths = [
                mmcv.imrescale(f_depth, results['scale_factor'], interpolation='bilinear')
                for f_depth in results['f_depths']
            ]
        else:
            depth_size = (results['img_shape'][1], results['img_shape'][0])
            f_depths = [
                mmcv.imrescale(f_depth, depth_size, interpolation='bilinear')
                for f_depth in results['f_depths']
            ]
        results['f_depths'] = f_depths
    ...
    def __call__(self, results):
        ...
        if 'f_depths' in results:
            self._resize_f_depth(results)
        if 'f_rgbs' in results:
            self._resize_f_rgb(results)
        return results
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            # flip full depth
            if 'f_depths' in results:
                 results['f_depths'] = [mmcv.imflip(f_depth, direction=results['flip_direction'])
                                        for f_depth in results['f_depths']]
            # flip full rgbs
            if 'f_rgbs' in results:
                results['f_rgbs'] = [mmcv.imflip(f_rgb, direction=results['flip_direction'])
                                     for f_rgb in results['f_rgbs']]
        return results
...
class Pad(object):
    ...
    def __call__(self, results):
        ...
        # pad depth
        if 'depth' in results:
            results['depth'] = mmcv.impad(results['depth'], results['pad_shape'][:2])
        # pad full depth
        if 'f_depths' in results:
            results['f_depths'] = [mmcv.impad(f_depth, results['pad_shape'][:2])
                                   for f_depth in results['f_depths']]
        # pad full rgbs
        if 'f_rgbs' in results:
            results['f_rgbs'] = [mmcv.impad_to_multiple(f_rgb, results['pad_size_divisor'])
                                 for f_rgb in results['f_rgbs']]
        return results
...
class SegResizeFlipPadRescale(object):
    ...
    def __init__(self, scale_factor=1, ignore_list=[], seg_num=None):
    ...
    def __call__(self, results):
        ...
        if len(self.ignore_list) > 0:
            for index in range(self.seg_num):
                if index in self.ignore_list:
                    gt_seg[gt_seg == index] = 0
                else:
                    gt_seg[gt_seg == index] = results['ann_info']['cat2label'][index]
