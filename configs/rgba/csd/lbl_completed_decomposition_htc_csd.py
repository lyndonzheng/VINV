...
model = dict(
    ...
    rgb_completion=dict(
        ...
        loss_g=dict(
            ...
            loss_rec=dict(type='smoothl1loss', loss_weight=1.0, in_weight=6.0),
            loss_vgg=dict(type='smoothl1loss', loss_weight=10.0, in_weight=6.0)
        )
    ),
)
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True, with_label=True, with_mask=True, with_seg=True, poly2mask=False,
    ...
    dict(type='Collect',
         keys=['img', 'gt_bboxes', 'gt_labels', 'gt_masks', 'gt_f_bboxes', 'gt_f_masks',
               'f_rgbs', 'f_depths', 'p_orders', 'l_orders', 'gt_semantic_seg']),
]
# test_pipeline = [
#     dict(type='LoadImageFromFile'),
