...
model = dict(
    ...
    mask_refined_roi_extractor=dict(
        type='RefinedRoIExtractor',
        roi_layer=dict(type='RoIAlign', out_size=14, sample_num=2),
        out_channels=256,
        featmap_strides=[4, 8, 16, 32]),
    mask_refined_head=dict(
        type='FCNMaskHead',
        num_convs=4,
        in_channels=256,
        conv_out_channels=256,
    ...
    rgb_completion=dict(
        ...
        loss_g=dict(
                ...
                loss_rec=dict(type='smoothl1loss', loss_weight=1.0),
                loss_vgg=dict(type='smoothl1loss', loss_weight=10.0)
            )
        ),
    # mask_gan=dict(
    #     type='MASK_GAN',
...
test_cfg = dict(
    ...
    rcnn=dict(
        ...
        mask_thr_binary=0.5))
# dataset settings
dataset_type = 'SoscDataset'
# data_root = '/home/lyndon/lyndon/dataset/'
...
test_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True, with_label=True, with_mask=True, with_seg=False, poly2mask=False,
         ...
         with_l_order=False, with_p_order=True, ),
    dict(type='Resize', img_scale=(512, 512), keep_ratio=True),
    ...
    dict(type='DefaultFormatBundle'),
    dict(type='Collect',
         keys=['img', 'gt_f_bboxes', 'gt_f_masks', 'p_orders']),
]
# test_pipeline = [
#     dict(type='LoadImageFromFile'),
#     dict(
...
data = dict(
    ...
    test=dict(
        ...
        pipeline=test_pipeline))
# optimizer
optimizer = dict(type='SGD', lr=0.00, momentum=0.9, weight_decay=0.0001, branch_lr=0.02)
